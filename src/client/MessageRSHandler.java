...
public class MessageRSHandler extends MessageHandler {
	...
	public String sendMessage(ObjectInputStream in, ObjectOutputStream out, MyGitClient params) {
		...
		return "MessageRSHHandler:sendMessage:" + params.getLocalUser() + " " + params.getServerAddress() + " "
				+ (params.getPassword() == null ? "" : "-p " + params.getPassword()) + " -" + params.getOperation()
				+ " "
				+ (params.getRepOrFileName() == null
						? (params.getFileName() == null ? params.getRepName() : params.getFileName())
						: params.getRepOrFileName())
				+ (params.getUserId() == null ? " " : " " + params.getUserId());
	}
	...
	public String sendShareMessage(ObjectInputStream in, ObjectOutputStream out, MyGitClient params) {
		...
		if (result.contentEquals("OK"))
			// receive the files
			System.out.println(
					"O repositório " + params.getRepName() + " foi partilhado com o utilizador " + params.getUserId());
	...
	public String sendRemoveMessage(ObjectInputStream in, ObjectOutputStream out, MyGitClient params) {
		...
		if (result.contentEquals("OK"))
			// receive the files
			System.out.println("O repositório " + params.getRepName() + " deixou de ser partilhado com o utilizador "
					+ params.getUserId());
		else if (result.contentEquals("NOK")) {
			String error = "";
			try {
